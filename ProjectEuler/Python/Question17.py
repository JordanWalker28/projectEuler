...
for x in range(1,1001):
    s_sum=sum
    if len(str(x))==4: sum+=num_word_dict[1000]
    elif len(str(x))==3:
        hundreds=int(str(x)[0]+"00")
        if x%100!=0: sum+=num_word_dict[hundreds]+3
        else: sum+=num_word_dict[hundreds]
        tens=int(str(x)[1]+"0")
        if tens!=00: sum+=num_word_dict[tens]
        ones=int(str(x)[2])
        if ones!=0: sum+=num_word_dict[ones]
